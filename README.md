...
[![Base Guardian — Wallet intelligence and onchain security for Base](./public/preview.png)](https://baseguardian.rakibhq.xyz)
...
- **Wallet overview:** Analyze a Base wallet address or supported name and show recent activity, lifetime activity, estimated gas usage, and an activity score.
...
- **Approval Guardian:** Discover standard ERC-20 and NFT approvals, verify their current onchain state, and let the connected owner revoke permissions directly.
...
- Activity score with human-readable reasons and no security claim
...
- In-app ERC-20, ERC-721, and ERC-1155 approval discovery with complete/partial scan coverage
- Single and capability-aware atomic batch revoke transactions with Permit2 exposure flags
- revoke.cash retained as an advanced fallback for unsupported permission systems
...
- Users can scan any Base wallet address or supported name without connecting.
- Standard approval events are discovered through Alchemy logs and verified against current contract state.
- Only the connected matching wallet can revoke verified permissions; unsupported and partial cases retain a Base-specific revoke.cash fallback.
- Base Guardian never requests private keys or seed phrases. Wallet signatures are requested only for explicit revoke transactions.
